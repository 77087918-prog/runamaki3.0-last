...
@section('content')
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <div class="p-4 sm:p-6 border-b border-gray-200">
            <form action="{{ route('habilidades.buscar') }}" method="GET" class="flex flex-col sm:flex-row gap-3 sm:gap-4">
                <div class="flex-1">
                    <input type="text" 
                           name="q" 
                           value="{{ request('q') }}"
                           placeholder="{{ __('app.skills.marketplace.search') }}" 
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                </div>
                <div class="sm:w-64">
                    <select name="categoria" 
                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        <option value="">{{ __('app.skills.marketplace.all_categories') }}</option>
        ...
        <div class="p-4 sm:p-6">
            @if($habilidades->isEmpty())
                <div class="text-center py-12">
                    <p class="text-gray-500">{{ __('app.dashboard.no_activity') }}</p>
                </div>
            @else
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                    @foreach($habilidades as $habilidad)
                        <div class="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100">
                            ...
                            <div class="relative h-40 sm:h-48 overflow-hidden bg-gradient-to-br from-purple-100 to-indigo-100">
                                <img src="{{ $habilidad->imagen_url }}" 
                                     alt="{{ $habilidad->titulo }}"
                                     class="w-full h-full object-cover hover:scale-110 transition-transform duration-500"
                                     loading="lazy">
                                <div class="absolute top-2 sm:top-3 right-2 sm:right-3 bg-white/95 backdrop-blur-sm px-2 sm:px-3 py-1 sm:py-1.5 rounded-full text-xs sm:text-sm font-medium shadow-sm">
                            ...
                            <div class="p-4 sm:p-5">
                                <div class="flex items-start justify-between mb-3">
                                    <div class="flex-1 min-w-0">
                                        <h3 class="text-base sm:text-lg font-bold text-gray-900 hover:text-indigo-600 transition mb-2 line-clamp-2">
